...
bazel_deps_map = {
    ...
    "@com_google_absl//absl/log": "absl::log",
    ...
    "@org_tensorflow//tensorflow/lite/delegates/gpu/common:types": (
        "-ltensorflowlite",
        "Eigen3::Eigen",
    ),  # Was just the first thing in the list of deps for a buidl that failed cos it neede eigen
    ...
    "@org_tensorflow//tensorflow/lite/delegates/gpu:api": (
        ...
        "-ltensorflowlite_gpu_gl",
        "-lEGL",
    ),
}

# Rules defined in the mediapipe bazel build files that we don't have access to and instead implement internally
# We bascally change the provided string used as a key with the value in this map:
external_to_internal_deps_map = {
    # This is a single .proto file we would otherwise have to pull the whole of the protobuf repo in for
    # the file is installed by protobuf but withut the bazel rule that tells us what to do with it.
    # So this rule is added to the src tree under hacks by the bitbake recipe before we kick off the build
    "@com_google_protobuf//:any_proto": "//hacks:any_proto"
}


# Map to allow determining which rule a given file was generated by.
rule_for_src_file = {}

# Allows us to ignore/avoid printing warnings about files we can't find later on (becasue they don't exist yet and bazel config gives us no way to determine this)
...
def write_templated_file(template, output_path, substitutions, log):
    try:
        ...
        log.info(
            f"Could not generate templated file, the source file {template} wasn't found"
        )
        return
...
class Project:
    class Module:
        ...
        class AbstractNamedModuleRule:
            ...
            def __init__(self, module, only_a_wrapper=False, **kwargs):
                ...
                self._full_path = (
                    f"{module.path}:{self._name}" if module is not None else self._name
                )
                self.tools = get_arg_or_default("tools", [], **kwargs)

                self.visibility = get_arg_or_default("visibility", [], **kwargs)
                self._log_context = module._log_context.new(
                    f"{self.__class__.__name__}[{self._name}]"
                )
                self._log_context.info(
                    f"{self.__class__.__name__} -> full_path : {self.full_path}, kwargs: {kwargs}"
                )
                ...
                if not only_a_wrapper:
                    if self.full_path in module.rules:
                        if not self._is_sub_rule:
                            fail(
                                f"Duplicate rule! Module module: {module.path} already has a rule called {self.full_path}"
                            )
                        else:
                            self._log_context.info(
                                "Not raising error for duplicate subrule."
                            )
                    else:
        ...
        class AbstractRule(AbstractNamedModuleRule):
            ...
            def resolve(self):
                ...
                for s in self.srcs:
                    if s in self._module._src_provider_map:
                        ...
                        self._log_context.info(
                            f"src file {s} of rule {self.full_path} is provided by {dep.full_path}"
                        )
            ...
            def copy_files(self, dest_dir, log, headers_only=False):
                ...
                def copy_file(path):
                    global cmake_generated_files
                    if path.startswith("//"):
                        log.info(f"IMPLEMENT ME, can't handle path {path}")
                        return
                    src = module_path / path
                    if str(src) not in cmake_generated_files:
                        dst = dest_path / path
        ...
        class Alias(AbstractRule):
            ...
            def cmake_target_name(self):
                if self.actual is None:
                    raise RuntimeError(
                        f"actual has not been set for Alias {self.full_path}"
                    )
        ...
        class AbstractCMakeCompiledRule(AbstractCCRule):
            ...
            def __init__(self, module, cmake_method, **kwargs):
                ...
                cmake_method = (
                    self.CMAKE_METHOD_LIBRARY if self.linkshared == 1 else cmake_method
                )
            ...
            def cmakelists_content(self):
                ...
                add_library_objects = (
                    self.library_type == self.CMAKE_LIBRARY_SHARED
                    or self.cmake_method == self.CMAKE_METHOD_EXECUTABLE
                )
                ...
                content += self.get_cmake_target_link_libraries(interface)
            ...
            def get_cmake_target_link_libraries(self, interface):
                content = ""
                ...
                if len(self.deps) > 0:
                    content += (
                        f"target_link_libraries({self.cmake_target_name} {interface}\n"
                    )
                    for d in self.deps_as_rules.values():
                        ...
                        def add_target_link_library_deps(d):
                            ...
                            if d.cmake_target_name.startswith("@"):
                                if d.cmake_target_name in bazel_deps_map:
                                    ...
                                    self._log_context.warning(
                                        f"No entry in map for {d.cmake_target_name}"
                                    )

                            if isinstance(d, Project.Module.TransitiveProtos):
                                #content += d.cmakelists_deps_content
                                for dd in d.cmakelists_deps:
                                    cmake_deps.add(dd)

                            if not (
                                isinstance(d, Project.Module.AbstractCMakeCompiledRule)
                                or isinstance(d, Project.Module.CCProtoLibrary)
                            ):
                                return
                            if isinstance(d, Project.Module.CCBinary):
                                return

                            cmake_deps.add(d.cmake_target_name)
        ...
        class CCProtoLibrary(AbstractCCRule):
            ...
            def cmakelists_content(self):
                ...
                content = (
                    ...
                    + "_sources "
                    + rule_name
                    + "_headers ${CMAKE_CURRENT_SOURCE_DIR}/"
                    + self._module.path
                    + "/"
                    + self.srcs[0]
                    + ")\n"
                ...
                content += (
                    ...
                    + rule_name
                    + '_sources} PROPERTIES COMPILE_FLAGS " -Wno-maybe-uninitialized " )\n'
                )
                content += (
                    "INSTALL(FILES ${CMAKE_CURRENT_BINARY_DIR}/"
                    + f"{self._module.path}/{include_name} DESTINATION include/{self._module.path} OPTIONAL)\n\n"
                )
                if len(self.deps_as_rules) > 0:
                    content += "target_link_libraries(" + rule_name + "\n"
                    for d in self.deps_as_rules.values():
                        if not isinstance(d, Project.Module.CCProtoLibrary):
                            print(f"CCPROTO Ignoring {d.name} (type: {type(d)})")
                            continue
        ...
        class ConfigSetting(AbstractNamedModuleRule):
            def __init__(self, module, **kwargs):
                super().__init__(module, **kwargs)
                self.define_values = (
                    kwargs["define_values"] if "define_values" in kwargs else []
                )
        ...
        class CMakeCustomCommand(AbstractCCRule):
            ...
            def cmakelists_content(self):
                ...
                if len(self.srcs) > 0:
                    content += f"    DEPENDS {' '.join([rule.cmake_target_name for rule in self.deps_as_rules.values()])}\n"
                content += ")\n\n"

                content += f"add_custom_target({self.cmake_target_name} DEPENDS\n"
                content += f"{self.outs[0]}\n"
                content += ")\n\n"
                # Allows us to ignore/avoid printing errors we can't find later on (becasue they don't exist yet and bazel config gives us no way to determine this)
                cmake_generated_files.append(f"{self.module.path}/{self.outs[0]}")
                return content
        ...
        class DataAsCString:
            ...
            def __init__(self, module, **kwargs):
                ...
                if outs == None:
                    outs = [name]
                encode_as_c_string = clean_dep(
                    "//mediapipe/framework/tool:encode_as_c_string"
                )
        ...
        class ExpandTemplate(AbstractRule):
            def __init__(self, module, **kwargs):
                ...
                template_output_path = (
                    Path(output_path) / src_path / module.path / output_filename
                )
        ...
        class MediapipeOptionsLibrary(AbstractRule):
            def __init__(self, module, **kwargs):
                ...
                module.data_as_c_string(
                    name=self.name + "_inc",
                    srcs=[
                        proto_lib + "_transitive-transitive-descriptor-set.proto.bin"
                    ],
                ...
                module.cc_library(
                    ...
                    deps=[
                        ...
                        f":{proto_lib.replace("_proto", "_cc_proto")}",
                    ]
                    + self.deps,
                    alwayslink=1,
                    visibility=self.visibility,
            ...
            def __init__(self, module, **kwargs):
                ...
                module.cc_binary(
                    ...
                    deps=[
                        ...
                        f":{name}_gather_cc_protos",
                    ],
                )

                def cmd(rule):
                    proto_source = f"{rule.module.path}/{graph}"
                    proto_output = f"{rule.module.path}/{output_name}"
                    return f"{name}_text_to_binary_graph --proto_source={proto_source} --proto_output={proto_output}"
        ...
        class MediapipeProtoLibrary(AbstractRule):
            def __init__(self, module, **kwargs):
                ...
                if def_cc_proto:
                    ...
                    module.mediapipe_cc_proto_library(
                        **provided_args(
                ...
                if def_options_lib:
                    ...
                    module.mediapipe_options_library(
                        **provided_args(
            ...
            def __init__(self, module, **kwargs):
                ...
                module.expand_template(
                    ...
                    substitutions={
                        ...
                        "{{SUBGRAPH_INC_FILE_PATH}}": module.package_name()
                        + "/"
                        + graph_base_name
                        + ".inc",
                    },
        ...
        class TransitiveDescriptorSet(AbstractRule):
            ...
            def on_resolve(self):
                ...
                """
                self._log_context.info(
                    f"Resolving transitive descriptors for {self.full_path}"
                )
                resolved = []
                ...
                def find_descriptors(rule):
                    ...
                    if isinstance(rule, Project.Module.ProtoLibrary):
                        ...
                        self._log_context.info(
                            f"    {rule.full_path} has {rule.out[0]}"
                        )
        ...
        class TransitiveProtos(AbstractRule):
            ...
            def on_resolve(self):
                ...
                def find_protos(rule):
                    ...
                    if isinstance(rule, Project.Module.CCProtoLibrary):
                        self.protos.append(rule)
                        self._log_context.info(
                            f"    {rule.full_path} has {rule.srcs[0]}"
                        )
            ...
            # @property
            # def cmakelists_deps_content(self):
            #     content = ""
            #     for p in self.protos:
            #         content += f"{p.cmake_target_name}\n"
            #     return content
            
            @property
            def cmakelists_deps(self):
                return [ p.cmake_target_name for p in self.protos ]
        ...
        def __init__(self, parent_log_context, path):
            ...
            self._log_context.info(f" === Loading module {path} ===\n")
            self.rules = {}
            spec = spec_from_loader(
                "Module", SourceFileLoader("Module", f"{path}/BUILD")
            )
            ...
            module.proto_library = self.proto_library
            module.select = self.select
            module.selects = self.selects
            module.test_suite = self.test_suite

            self._log_context.info(f" === Exec module... ===\n")
            try:
                spec.loader.exec_module(module)
            except Exception as e:
                fail(f"Error running exec_module: {e}")

            self._log_context.info(f" === Resolving out files... ===\n")
            self.resolve_src_out_file_dependencies()
            self._log_context.info(f"\n=== Parsing module {path} complete ===\n\n")
        ...
        def resolve_src_out_file_dependencies(self):
            ...
            Addiionally populate global rule_for_src_file to allow finding which rule a given file is provided by
            """
            global rule_for_src_file
            ...
            for rule in self.rules.values():
                ...
                if not isinstance(rule, Project.Module.AbstractRule):
                    continue
                self._log_context.info(
                    f"resolve_src_out_file_dependencies() resolving {rule.name}, outs: {rule.outs}"
                )
                for out in rule.outs:
                    if out in out_rule_map:
                        conflicting_rule = out_rule_map[out].name
                        fail(
                            f"Duplicate out file! {out}:  provided by {rule_name} conflicts with that of {conflicting_rule}!"
                        )
                    else:
                        out_rule_map[out] = rule_name

                    if out in rule_for_src_file:
                        conflicting_rule = out_rule_map[out].name
                        fail(
                            f"Duplicate out file! {out}:  provided by {rule_name} conflicts with that of {conflicting_rule}!"
                        )
                        rule_for_src_file[out] = rule
            ...
            for rule in self.rules.values():
                ...
                for src in rule.srcs:
                    if src in out_rule_map:
                        ...
                        self._log_context.info(
                            f"resolve_src_out_file_dependencies() {rule_name} depends on {dependency} (for src file {src})"
                        )
        ...
        def package(self, default_visibility=[], **kwargs):
            self._log_context.info(
                f"package() default_visibility: {default_visibility}"
            )

        def exports_files(self, files=[], srcs=[], visibility=[]):
            self._log_context.info(
                f"exports_file(files: {files}, srcs: {srcs}, visibility: {visibility}) Implement me!"
            )
        ...
        def cc_test_with_tflite(self, **kwargs):
            self._log_context.info(
                f"cc_test_with_tflite(**kwargs: {kwargs}) Implement me!"
            )
        ...
        def filegroup(self, **kwargs):
            self._log_context.info(
                f"mediapipe_cc_test(**kwargs: {kwargs}) Implement me!"
            )
        ...
        def load(self, module, rule, another_rule=None):
            ...
            # We don't attempt to load any bazel modules, atm just implement anything needed in this file.
            pass
        ...
        def mediapipe_register_type(self, **kwargs):
            self._log_context.info(
                f"mediapipe_register_type(**kwargs: {kwargs}) Implement me!"
            )
            ...
            if self._selects is None:
                ...
                class Selects:
                    ...
                    def config_setting_group(self, **kwargs):
                        self._log_context.info(
                            f"config_setting_group(kwargs: {kwargs}) Implement me!"
                        )
                    ...
                    def with_or(self, obj):
                        ...
                        self._log_context.info(
                            f"selects.with_or(what: {obj}) Implement me! returning []"
                        )
                        return []
            ...
            if self._more_selects is None:
                ...
                class MoreSelects:
                    ...
                    def config_setting_negation(self, **kwargs):
                        self._log_context.info(
                            f"config_setting_negation(kwargs: {kwargs}) Implement me!"
                        )
    ...
    def __init__(self, top_level_build_path, rules):
        ...
        for a in rules:
            ...
            try:
                ...
                deps_log.warning(
                    f"!!! Missing dependency !!! : {full_path}, we have: {top_level_build_module.rules.keys()}"
                )
            ...
            def resolve_deps(rule, depth):
                if rule.full_path in deps_resolved:
                    deps_log.info(
                        f"resolve_deps() {' '*depth} {rule.full_path} already resolved."
                    )
                    return
                rule.required = True
                ...
                if isinstance(rule, Project.Module.MediapipeOptionsLibrary):
                    deps_log.info(
                        f"resolve_deps() {' '*depth} {rule.full_path} is an instance of Project.Module.MediapipeOptionsLibrary!"
                    )
                ...
                deps_log.info(
                    f"resolve_deps() {'    '*depth} {rule.full_path} (type: {type(rule)}) depends on:"
                )
                deps_log.info(f"deps: {rule.deps}, tools: {rule.tools}")

                def resolve_dep(d, is_tool=False):
                    global external_to_internal_deps_map
                    if d in external_to_internal_deps_map:
                        mapped = external_to_internal_deps_map[d]
                        deps_log.info(
                            f"Using internally re-mapped {mapped} instead of {d}"
                        )
                        d = mapped

                    tool_str = "(is a tool)" if is_tool else ""
                    deps_log.info(
                        f"resolve_deps() {'    '*depth} {d} {tool_str} resolving..."
                    )
                    try:
                        ...
                        if dependency_rule:
                            ...
                            deps_log.info(
                                f"resolve_deps() {'    '*depth}  {dependency_rule.full_path}"
                            )
                            resolve_deps(dependency_rule, depth + 1)
                        else:
                            deps_log.info(
                                f"resolve_deps() {'    '*depth}  self.rule_from_dep_string() yield'd None for rule.module: {rule.module.path}, d: {d}"
                            )

                    except Exception as e:
                        deps_log.info(
                            f"resolve_deps() {'    '*depth} module {rule.module.path}, rule {rule.name} depends on {d} but it could not be resolved (exception: {e})"
                        )
        ...
        print(f"cmake_generated_files: {cmake_generated_files}")
        ...
        for name in list(deps_resolved.keys()):
            ...
            match name:
                case "mediapipe/framework/profiler:graph_profiler":
                    actual = self.rule_from_dep_string(
                        rule.module, ":graph_profiler_stub"
                    )
                ...
                case "mediapipe/gpu:gpu_shared_data_internal":
                    actual = self.rule_from_dep_string(
                        rule.module, ":gpu_shared_data_internal_actual"
                    )

                case "mediapipe/framework/port:threadpool_impl_default_to_google":
                    actual = self.rule_from_dep_string(
                        rule.module, "//mediapipe/framework/deps:threadpool"
                    )

                case _:
                    deps_log.info(
                        f"resolve_aliases() IMPLEMENT ME, no resolution for Alias {name}"
                    )
            ...
            if actual is not None:
                ...
                rule.actual = actual
                deps_log.info(
                    f"resolve_aliases() actual set to {actual.name} for rule {name}"
                )

    def rule_from_dep_string(self, module, str):
        global bazel_deps_map
        key = None
        try:
            if str.startswith("@"):
                if str not in bazel_deps_map:
                    self._log_context.warning(
                        f"\tProject.rule_from_dep_string() Unknown external dependency: {str}"
                    )
                else:
                    self._log_context.info(
                        f"\tProject.rule_from_dep_string() External dependency: {str}"
                    )
            ...
            if ":" in str:
                ...
                module = (
                    module
                    if module_path == ""
                    else self.get_or_load_module(module_path)
                )
                if module is None:
                    self._log_context.warning(
                        "rule_from_dep_string() Module not loaded, cannot find rule!"
                    )
                    return None
                ...
                def resolve_alias(rule):
                    if isinstance(rule, Project.Module.Alias):
                        self._log_context.info(
                            f"\tProject.rule_from_dep_string()::resolve_alias() resolving alias : {rule.name}, actual: {rule.actual}"
                        )
            ...
            if str not in ("protoc",):
                self._log_context.warning(
                    f"\tProject.rule_from_dep_string() Implement me! Don't know how to handle dependency: {str}"
                )
        except Exception as e:
            self._log_context.warning(
                f'\tProject.rule_from_dep_string() Failed for: "{module.path}", name: {str}, key: "{key}"'
            )
            self._log_context.info(
                f"\tProject.rule_from_dep_string() module.rules : {module.rules}"
            )
            raise e
    ...
    def get_or_load_module(self, module_path):
        ...
        if module_path in self.modules:
            self._log_context.info(
                f"get_or_load_module() returning existing instance of module with module_path: {module_path}"
            )
            return self.modules[module_path]
        self._log_context.info(
            f"get_or_load_module() trying to load module with module_path: {module_path}"
        )
        ...
        if module is not None:
            ...
            self._log_context.info(
                f"get_or_load_module() loaded new module: {module_path}"
            )
        else:
            self._log_context.warning(
                f"get_or_load_module() failed to load module for : {module_path}"
            )
        return module
    ...
    if info.module:
        for rule in info.module.rules.values():
            ...
            if isinstance(rule, Project.Module.AbstractCMakeCompiledRule):
                ...
                cmake_log.info(
                    f"MediapipeOptionsLibrary is by product of proto_lib: {rule.proto_lib.name}, with the following deps:"
                )
                for d in rule.proto_lib.deps:
                    cmake_log.info(f"   {d}")
            else:
                # cmake_log.info(
                #    f"IMPLEMENT ME! We need this ({rule.full_path}) no implementation for  {rule.__class__.__name__}"
                # )
                pass
    ...
    if dpg is None:
        ...
        with dpg.window(label="Window", tag="Window"):
            dpg.add_collapsing_header(
                label="Log Contexts:", tag="log_context", default_open=True
            )
